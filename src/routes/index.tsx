...
function Index() {
  ...
  return <main className="bg-background text-foreground">
    ...
    <section id="home" className="grain relative flex min-h-[92svh] items-center overflow-hidden bg-primary text-primary-foreground">
      ...
      <div className="absolute inset-0 bg-primary/60 sm:bg-primary/52"/><div className="absolute inset-0 bg-linear-to-r from-primary/80 via-primary/30 to-transparent"/>
    ...
    <section className="relative min-h-[70svh] overflow-hidden"><img src={nightExteriorImage} alt="Germinnaa's illuminated green entrance and striped awnings at night" width="768" height="1024" loading="lazy" className="absolute inset-0 h-full w-full object-cover object-center sm:object-[center_58%]"/><div className="absolute inset-0 bg-primary/55"/><div className="relative z-10 flex min-h-[70svh] items-center justify-center px-5 text-center text-primary-foreground"><h2 className="text-5xl leading-none sm:text-8xl">Come Hungry.<br/><em>Leave With a Memory.</em></h2></div></section>
    ...
    <section id="gallery" className="px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto max-w-[1500px]"><div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs uppercase tracking-[.24em] text-accent">Visual diary</p><h2 className="mt-4 text-5xl text-primary sm:text-7xl">A Little Look Inside.</h2></div><a href="https://www.instagram.com/germinnaa/" target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-primary hover:text-accent"><Instagram className="size-4"/> Follow Our Journey <ArrowRight className="size-4"/></a></div><div className="mt-14 grid auto-rows-[190px] grid-cols-2 gap-3 sm:auto-rows-[250px] md:grid-cols-4">{[[croissantImage,"A berry-topped croissant at Germinnaa","row-span-2"],[berrySlushImage,"A berry slush garnished with mint and lime","col-span-1"],[snackPlatterImage,"A sharing platter with crisp bites and fries","col-span-1"],[archedInteriorImage,"Germinnaa dining room framed by a decorated arch","row-span-2"],[foodImage,"Grilled skewers with vegetables and fresh salad","col-span-2"],[dessertImage,"Freshly made coffee with biscuits and roasted beans","col-span-1"],[stuffedDishImage,"Stuffed baked dishes with fresh ribbon salad","col-span-1"],[ambienceImage,"The emerald and ivory dining room at Germinnaa","col-span-2"],[burgerImage,"Germinnaa burger served with fries","col-span-1"],[sandwichImage,"A toasted sandwich served with fries","col-span-1"],[exteriorImage,"The welcoming green exterior of Germinnaa","col-span-2"],[heroImage,"Germinnaa signature pizza with rocket and parmesan","col-span-2"]].map(([src,alt,classes],i) => <div key={i} className={`group overflow-hidden ${classes}`}><img src={src} alt={alt} loading="lazy" width="1000" height="1000" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105"/></div>)}</div></div></section>
