...
const heroImage = archedInteriorAsset.url;
...
function Index() {
  ...
  return <main className="bg-background text-foreground">
    ...
    <section className="relative min-h-[70svh] overflow-hidden"><img src={exteriorImage} alt="The emerald green exterior of Germinnaa in Southern Avenue" width="1024" height="768" loading="lazy" className="absolute inset-0 h-full w-full object-cover"/><div className="absolute inset-0 bg-primary/55"/><div className="relative z-10 flex min-h-[70svh] items-center justify-center px-5 text-center text-primary-foreground"><h2 className="text-5xl leading-none sm:text-8xl">Come Hungry.<br/><em>Leave With a Memory.</em></h2></div></section>
    ...
    <section id="gallery" className="px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto max-w-[1500px]"><div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs uppercase tracking-[.24em] text-accent">Visual diary</p><h2 className="mt-4 text-5xl text-primary sm:text-7xl">A Little Look Inside.</h2></div><a href="https://www.instagram.com/germinnaa/" target="_blank" rel="noreferrer" className="flex items-center gap-2 text-sm text-primary hover:text-accent"><Instagram className="size-4"/> Follow Our Journey <ArrowRight className="size-4"/></a></div><div className="mt-14 grid grid-cols-2 gap-3 md:grid-cols-4">{[[heroImage,"Germinnaa dining room framed by a decorated arch","row-span-2 aspect-[3/5] md:aspect-auto"],[foodImage,"Germinnaa grilled skewers with fresh salad","aspect-square md:col-span-2"],[dessertImage,"Freshly made coffee with biscuits and roasted beans","aspect-square"],[ambienceImage,"The emerald and ivory dining room at Germinnaa","aspect-square"],[burgerImage,"Germinnaa burger served with fries","aspect-square"],[sandwichImage,"A toasted Germinnaa sandwich served with fries","aspect-square"],[exteriorImage,"The welcoming green exterior of Germinnaa","aspect-square md:col-span-2"]].map(([src,alt,classes],i) => <div key={i} className={`group overflow-hidden ${classes}`}><img src={src} alt={alt} loading="lazy" width="1000" height="1000" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105"/></div>)}</div></div></section>
